...
class EGNNLayer(nn.Module):
    ...
    def forward(self, x_query, x_ref, h_query, h_ref, neighbor=None, mask=None):
        ...
        if self.neighbor_mode != 'all':
            x_ref = x_ref.unsqueeze(2).repeat(1, 1, n_query, 1)  # batch_size, 3, n_query, n_ref
            h_ref = h_ref.unsqueeze(2).repeat(1, 1, n_query, 1)  # batch_size, feat_dim, n_query, n_ref
            x_ref = x_ref.gather(dim=3, index=neighbor.repeat(1, xyz_dim, 1, 1))
            h_ref = h_ref.gather(dim=3, index=neighbor.repeat(1, feat_dim, 1, 1))
            n_ref = neighbor.shape[2]

        x_query = x_query.unsqueeze(3).repeat(1, 1, 1, n_ref)  # batch_size, 3, n_query, n_ref
        h_query = h_query.unsqueeze(3).repeat(1, 1, 1, n_ref)  # batch_size, feat_dim, n_query, n_ref

        x_diff = x_query - x_ref  # batch_size, 3, n_query, n_ref
        x_dist2 = (x_diff * x_diff).sum(dim=1, keepdim=True)  # batch_size, 1, n_query, n_ref

        edge_feat = torch.cat([h_query, h_ref, x_dist2], dim=1)  # batch_size, feat_dim*2+1, n_query, n_ref
        m_edge = self.edge_mlp(edge_feat.reshape(batch_size, -1, n_query * n_ref))  # batch_size, m_edge_dim, n_query, n_ref

        x_res_weight = self.coord_mlp(m_edge)
        x_res_weight = x_res_weight.reshape(batch_size, -1, n_query, n_ref) # batch_size, 1, n_query, n_ref
        ...
        if self.gather_mode == 'sum':
            ...
            if mask is None:
                ...
                m_node = (m_edge * mask.unsqueeze(1)).sum(dim=3) / mask.unsqueeze(1).sum(dim=3)
            ...
            m_node = (m_edge * attn).sum(dim=3)  # batch_size, m_edge_dim, n_query
        else:
            raise NotImplementedError

        node_feat = torch.cat([h_query, m_node], dim=1)  # batch_size, m_edge_dim + feat_dim, n_query
